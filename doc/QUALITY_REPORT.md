...
设置按钮将两条生命周期分开处理：尚未创建设置窗口时，通过 SwiftUI
`OpenSettingsAction` 创建 `Settings` 场景；设置场景随后注册其真实 `NSWindow`。
重复点击设置时，应用激活自身，再将已有设置窗口设为 key 并前置。

真机回归覆盖：全新进程从未创建设置窗口时首次点击、设置窗口已存在并被 Finder 与
监控面板覆盖时再次点击、关闭设置后再次点击。三种场景均立即显示
`com_apple_SwiftUI_Settings_window`，未创建重复窗口。
