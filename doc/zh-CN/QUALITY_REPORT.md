...
适用版本：1.0.0（Build 1）
应用代码基线：`24e6c27`
结论：本地工程与自动化发布门禁通过；冷后台 CPU 与可见态内存达标；快速交互 CPU 峰值，以及独立卡片关闭后的 CPU 和 physical footprint 回落仍需优化；多设备、长时间及 Apple 发行链路仍待执行
...
采样协调器在一个 actor 中管理生命周期，并使用结构化并发同时读取四类指标。设计上只有监控面板可见时才物化完整快照和图表历史；本轮发现独立窗口关闭路径没有正确回到该隐藏状态，详见第 7 节。
...
该矩阵表明：冷后台平均 CPU 为 0.290%，满足低于 1% 的产品目标；前台静置平均 1.724%，窗口移动平均 2.873%。常规连续快速交互平均 7.345%、峰值 13.973%，仍存在短时两位数峰值；反复抓取完整 AX 树的自动化压力场景峰值 25.310%，只作为测试上界，不能当作普通鼠标使用数据。卡片关闭后 CPU 平均仍为 2.745%，没有回到冷后台水平。可见态 physical footprint 峰值 38.6 MiB，低于 60 MB 产品预算。
...
上述机制已通过单元测试、构建和代码路径检查。本轮已在解锁真机上完成独立监控卡片窗口的“冷启—打开—交互—关闭—稳定”测量，替换此前因锁屏而缺失的数据。
...
最后 20 秒的 CPU 平均为 2.745%、P95 为 3.189%，高于冷后台平均 0.290%，因此关闭后的 CPU 回落未通过。physical footprint 平均为 38.9 MiB、峰值为 39.0 MiB，比同轮冷启后台平均 16.5 MiB 高约 22.3 MiB，同样未达到预期回落。退出并重新启动后，CPU 平均恢复到 0.211%，physical footprint 平均恢复到 16.7 MiB。

因此本项的正式状态是：展示数据与图表数组的清理机制已实现，但独立窗口关闭路径的进程 CPU 与 physical footprint 回落均未通过。结合 `NSWindow.isReleasedWhenClosed = false` 且窗口由 `AppDelegate` 持有的实现，当前结果与关闭后 SwiftUI 内容仍驻留、可见性状态未切回后台相符；这仍是基于代码和测量的推断，不替代 Instruments 证据。还需使用 Allocations、Leaks、Time Profiler 和内存图区分仍存活对象、SwiftUI/AppKit 缓存及分配器未归还页面。默认菜单栏弹出式卡片也需在相同口径下独立复测，不能用本次独立窗口结果代替。
...
- 修复并复测独立卡片关闭后的 CPU 与 physical footprint 回落；当前稳定 CPU 平均约 2.7%、footprint 约 39 MiB，冷启基线约 0.3% / 17 MiB。
